...
int main(int argc, char **argv) {
	...
	hdfs_oauth oauth = NULL;

	oauth = (hdfs_oauth)calloc(1, sizeof(struct hdfs_oauth_internal));

	oauth->client_id = strdup("");
	oauth->expires_at = 0;
	oauth->access_token = strdup("");
	oauth->refresh_token = strdup("");

	oauth->refresh_url = strdup("");
    ...
    fs = shdfsConnectWithOAuth(argv[5], 443, oauth);
    if (!fs) {
    ...
    append = false;//atoi(argv[5]);
    if (!append) {
		//hdfsDelete(fs, writeFileName, 0);
